...
class Encoder(layers.Layer):
    def __init__(self, x_dim, encoder_hiddens, z_dims, name='encoder', **kwargs):
        super(Encoder, self).__init__(name=name, **kwargs)

        self.encoder_inputs = keras.Input(shape=x_dim)
        self.encoder_layers = []
        for i, units in enumerate(encoder_hiddens):
            self.encoder_layers.append(layers.Dense(units, activation='relu', name='encoder_layer_%d' % i))
        self.z = layers.Dense(z_dims, activation='relu', name='z')
...
class Decoder(layers.Layer):
    def __init__(self, x_dim, decoder_hiddens, z_dims, name='decoder', **kwargs):
        super(Decoder, self).__init__(name=name, **kwargs)
        
        self.decoder_layers = [layers.Dense(z_dims, activation='relu', name='z')]
        ...
        self.rec_x = layers.Dense(x_dim, name='rec_x')
...
class AE(Model):
    def __init__(self, 
        ...
        self.encoder = Encoder(n_features, encoder_hiddens, latent_dims, name='encoder')
        self.decoder = Decoder(n_features, decoder_hiddens, latent_dims, name='decoder')
