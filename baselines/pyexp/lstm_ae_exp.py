...
def main(config_, i):
    ...
    for syn_config in syn_configs:
        if not os.path.exists('%s/spat_%.2f_slice_%.2f/cluster_perf.json' % (log_dir, syn_config['spat_frac'], syn_config['slice_frac'])):
            ...
            cluster_evaluate(syn_X, syn_y, config, scaler, model.get_score, NF=False, rnn_based=True, save_dir='%s/spat_%.2f_slice_%.2f' % (log_dir, syn_config['spat_frac'], syn_config['slice_frac']))
        ...
        if syn_config['slice_frac'] == 0.05 and not os.path.exists('%s/spat_%.2f_slice_%.2f/seg_perf.json' % (log_dir, syn_config['spat_frac'], syn_config['slice_frac'])):
            kde_segment_evaluate(syn_X, syn_interpret_label, config, syn_config, scaler, log_dir, NF=False, rnn_based=True)
