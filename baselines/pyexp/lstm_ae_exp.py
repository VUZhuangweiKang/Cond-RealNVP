...
def main(config_, i):
    ...
    log_dir = '../../experiments/Inrix/logs/LSTM_AE/run%d/%s/%d' % (i, cluster_method, config.cid)
    model_dir = '../../experiments/Inrix/models/LSTM_AE/%s/%d' % (cluster_method, config.cid)
    ...
    model = AE(n_features=len(segments), 
                ...
                sw_len=config.window, 
                steps=config.steps, 
                model_dir='%s/lstm_ae'%model_dir)
    model.compile()
...
#             metrics, scores, cluster_labels = cluster_evaluate(syn_X, syn_y, config, scaler, model.get_score, NF=False, rnn_based=True,
...
#             results, segment_labels = kde_segment_evaluate(syn_X, syn_interpret_label, config, syn_config, scaler, log_dir, NF=False, rnn_based=True)
#             evaluate(syn_X, syn_interpret_label, segment_labels, log_dir, config, syn_config)
    ...
    # print(results)
    roc_evaluate(X_test, y_test_, config, scaler, model.get_score, save_dir=log_dir, rnn_based=True)
