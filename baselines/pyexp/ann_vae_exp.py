...
configs_ = [
    ...
    },
    {
        ...
        "year": 2019,
        "batch_size": 64,
        "encoder_hiddens": [128, 64],
        "decoder_hiddens": [64, 128],
        "latent_dims": 10,
        "epochs": 300,
        "lr": 1e-4,
        "early_stop": 30
...
def main(config_, i):
    ...
    log_dir = '../../experiments/Inrix/logs/ANN_VAE/run%d/%s/%d' % (i, cluster_method, config.cid)
    model_dir = '../../experiments/Inrix/models/ANN_VAE/%s/%d' % (cluster_method, config.cid)
    ...
    model = VAE(n_features=len(segments),
                 ...
                 beta=1.0, 
                 model_dir='%s/ann_vae'%model_dir)
    model.compile()
    
    es = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=config.early_stop)
